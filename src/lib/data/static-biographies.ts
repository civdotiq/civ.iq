 ...
 *
 * SERVER-ONLY. The corpus is a single 1.34 MB JSON covering all 520 members.
 * Importing any of the runtime exports below from a `'use client'` module puts
 * that whole file in a browser chunk (~209 KB gzipped + a 1.2 MB main-thread
 * JSON.parse) to render one ~2 KB record — which is exactly what
 * BiographyCard used to do. Client components must go through
 * `GET /api/representative/[bioguideId]/biography` instead. The
 * `StaticBiography` type is safe to `import type` from anywhere.
 *
