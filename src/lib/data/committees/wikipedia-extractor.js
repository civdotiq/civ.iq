        ...
        committeeSection.querySelectorAll('li').forEach(li => {
          ...
          links.forEach(link => {
            if (link.href.includes('/wiki/') && !link.href.includes(':')) {
              ...
              const role = li.textContent.includes('Chair') ? 'Chair' : 
                         li.textContent.includes('Ranking') ? 'Ranking Member' : 'Member';
...
function formatMembersForTypeScript(members) {
  return members.map(member => {
    ...
    const party = member.role === 'Chair' ? 'R' : 
                  member.role === 'Ranking Member' ? 'D' : 'R';
