...
export class BillSummaryFallbacks {
  ...
  static async executeFallbackChain(
    ...
    if (opts.useCongressionalSummary) {
      try {
        ...
        errors.push(`Congressional summary failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
    ...
    if (opts.useSimpleExtraction) {
      try {
        ...
        errors.push(`Simple extraction failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  ...
  private static async createKeywordBasedSummary(
    ...
    try {
      ...
      const summary = `This bill ${mainAction.toLowerCase()}. ${keyActions.slice(1, 3).map(action => 
        `It also ${action.toLowerCase()}`
      ).join('. ')}.`;
   ...
   */
  private static createBasicFallbackSummary(
    billMetadata: { number: string; title: string; congress: number; chamber: string }
  ): BillSummary {
  ...
  private static extractKeyPointsFromText(text: string): string[] {
    ...
    return sentences.slice(0, 5).map(sentence => 
      sentence.trim().replace(/^(this|the|a|an)\s+/i, '').trim()
    ...
    groupPatterns.forEach(pattern => {
      ...
      if (matches) {
        matches.forEach(match => {
          ...
          if (normalized.includes('student') || normalized.includes('school') || normalized.includes('teacher')) {
            foundGroups.add('Students and educators');
          } else if (normalized.includes('worker') || normalized.includes('business') || normalized.includes('employ')) {
          ...
          } else if (normalized.includes('famil') || normalized.includes('children') || normalized.includes('parent')) {
            foundGroups.add('Families and children');
          } else if (normalized.includes('farm') || normalized.includes('agriculture') || normalized.includes('rural')) {
            foundGroups.add('Farmers and rural communities');
          } else if (normalized.includes('health') || normalized.includes('patient') || normalized.includes('doctor') || normalized.includes('hospital')) {
            foundGroups.add('Healthcare providers and patients');
          } else if (normalized.includes('taxpayer') || normalized.includes('citizen') || normalized.includes('resident')) {
  ...
  static validateFallbackSummary(summary: BillSummary): {
    ...
    const hasPlaceholders = placeholderPhrases.some(phrase => 
