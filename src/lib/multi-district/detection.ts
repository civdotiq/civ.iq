...
export async function checkMultiDistrict(zipCode: string): Promise<MultiDistrictResponse> {
  try {
    const response = await fetch(`/api/representatives-multi-district?zip=${encodeURIComponent(zipCode)}`);
    ...
    structuredLogger.info('Multi-district check completed', {
      zipCode,
    ...
    structuredLogger.error('Multi-district check failed', error as Error, { zipCode });
...
export async function getDistrictsForZip(
  ...
  try {
    ...
    structuredLogger.info('Districts retrieved for ZIP', {
      zipCode,
      selectedDistrict,
      districtCount: data.districts.length,
    ...
    structuredLogger.error('Failed to get districts for ZIP', error as Error, { 
      zipCode, 
      selectedDistrict 
    });
...
export function formatMultiDistrictSummary(districts: DistrictInfo[]): string {
  ...
  const stateGroups = districts.reduce((acc, district) => {
...
export function quickMultiDistrictCheck(zipCode: string): boolean {
  ...
  const commonMultiDistrictPrefixes = [
    ...
    '112', '113', '114', // Queens/Brooklyn
    '900', '902', '906', '917', // LA area
    '606', '607', '608', // Chicago
    '770', '772', '773', // Houston
    '480', '481', '482', // Phoenix
    '191', '190', // Philadelphia
    '021', '022', '024', // Boston
    '200', '202', '204', // DC area
  ];
