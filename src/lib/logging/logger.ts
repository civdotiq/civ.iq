...
const logger = winston.createLogger({
  ...
  transports: [
    ...
    new winston.transports.Console({
      format: process.env.NODE_ENV === 'production' ? winston.format.json() : structuredFormat,
    }),
  ],
});
...
export class StructuredLogger {
  ...
  private constructor() {
    ...
    // Bind all methods to preserve 'this' context when methods are destructured or passed as callbacks
    this.error = this.error.bind(this);
    this.warn = this.warn.bind(this);
    this.info = this.info.bind(this);
    this.debug = this.debug.bind(this);
    this.extractRequestMetadata = this.extractRequestMetadata.bind(this);
  }
  ...
  security(
    ...
    const requestMeta = this.extractRequestMetadata(request);
    const level =
      event.includes('failure') || event.includes('denied') || event.includes('limit')
        ? 'warn'
        : 'info';
