 ...
 *
 * Analyzers fan out to many *independent* network and classification calls
 * (one bill, one recipient at a time) that were historically awaited
 * serially — the dominant cause of the uniform 55s analyzer timeouts. A
 * bounded worker pool runs them in parallel without unleashing unbounded
 * concurrency on the CPU-bound zero-shot classifier or rate-limited gov
 * APIs. Results map 1:1 to inputs by index, so callers can zip them back
 * against the source list. A rejecting `fn` rejects the whole call (same
 * failure semantics as `Promise.all`); wrap `fn` if you want per-item
 * fail-soft behavior.
 */
