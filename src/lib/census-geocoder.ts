...
export async function geocodeAddress(
  ...
  try {
    const params = new URLSearchParams({
      ...
      layers: '54', // Congressional Districts 118th Congress
      format: 'json'
    });
    ...
    const validMatches = data.result.addressMatches.filter(match => 
      match.geographies?.congressionalDistricts?.length > 0
    );
    ...
    structuredLogger.info('Census Geocoder success', {
      ...
      districts: validMatches.map(m => 
        m.geographies.congressionalDistricts.map(d => `${d.STATE}-${d.CD}`)
      ).flat()
    });
