...
export const DATASET_REGISTRY: DatasetGenerator[] = [
  ...
  },
  {
    ...
    description:
      'Complete Senate LDA quarterly reports (LD-2) for the corpus window, rolled up per organization per quarter. The full record, not a sample — totals, rankings and market shares are safe to compute. Amounts are plausibility-gated and amendments deduped.',
    source: 'Senate LDA API (CIV.IQ corpus mirror)',
    sourceUrl: 'https://lda.gov',
    approximateRows: '~124,000',
    freshness: 'Corpus refreshed weekly',
    columnLabels: [
      'Client + Quarter',
      'Client',
      'Quarter',
    ...
    keyColumn: 'clientQuarter',
    // Row-level change detection would write ~16 MB to Redis on every download.
    // The corpus carries its own generatedAt stamp and freshness canary.
    skipDiff: true,
