...
function calculateTrend(industryData: unknown): 'up' | 'down' | 'stable' {
  ...
  const recentTotal = recentContributions.reduce((sum: number, c: unknown) => {
    const contrib = c as { amount?: number };
    return sum + (contrib.amount || 0);
  }, 0);
  const olderTotal = olderContributions.reduce((sum: number, c: unknown) => {
    const contrib = c as { amount?: number };
    return sum + (contrib.amount || 0);
  }, 0);
...
export default industryMapperExports;
