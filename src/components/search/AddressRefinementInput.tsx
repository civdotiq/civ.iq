...
export function AddressRefinementInput({
  ...
  return (
    <div className="bg-white border-2 border-civiq-blue p-4 sm:p-6 mx-auto max-w-lg animate-fade-in-up">
      ...
      <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
        ...
        <div className="flex gap-2 sm:gap-3 pt-2">
          ...
          >
            {isValidating ? (
              <>
                <div className="w-3.5 h-3.5 sm:w-4 sm:h-4 border-2 border-white/30 border-t-white animate-spin" />
          ...
          <button
            type="button"
            onClick={onCancel}
            className="px-4 sm:px-6 py-2.5 sm:py-3 bg-gray-100 text-gray-700 hover:bg-gray-200 transition-all duration-200 font-medium text-sm sm:text-base"
          >
            Cancel
          </button>
        </div>
        ...
        <div className="space-y-1.5 sm:space-y-2">
          {exampleAddresses.map((example, index) => (
            <button
              ...
              className="w-full text-left px-2.5 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-gray-600 hover:bg-gray-50 transition-colors"
            >
              {example}
            </button>
          ))}
      ...
      {/* Help Text */}
      <div className="mt-3 sm:mt-4 text-[10px] sm:text-xs text-gray-500 bg-gray-50 p-2.5 sm:p-3">
