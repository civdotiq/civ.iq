...
export async function GET(request: NextRequest) {
  try {
    ...
    structuredLogger.info('Processing enhanced search request', {
    ...
    if (actualCount < expectedCount) {
      structuredLogger.warn('Incomplete representative data', {
        operation: 'representatives_search',
    ...
    const response = {
      ...
      metadata: {
        ...
        warning: actualCount < expectedCount ? 'Incomplete representative data - some representatives may be missing' : null
      }
    }
    
    structuredLogger.info('Enhanced search completed successfully', {
      operation: 'representatives_search',
      searchType,
      totalFound: districtRepresentatives.length,
      district: `${districtInfo.state}-${districtInfo.district}`,
    ...
    structuredLogger.error('Enhanced search failed', error as Error, {
      operation: 'representatives_search'
    }, request)
