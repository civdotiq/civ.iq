...
function generateCampaignFinanceData(bioguideId: string, years: number = 6): CampaignFinanceData[] {
  ...
  const industries = [
    'Technology', 'Healthcare', 'Financial Services', 'Energy', 'Agriculture',
    'Defense', 'Education', 'Real Estate', 'Transportation', 'Manufacturing'
  ];
  ...
  for (let year = currentYear - years; year <= currentYear; year++) {
    for (let quarter = 1; quarter <= 4; quarter++) {
      ...
      const topIndustries = selectedIndustries.map((industry, index) => {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const summary = {
      ...
      averageQuarterlyRaising: Math.round(financeData.reduce((sum, d) => sum + d.totalRaised, 0) / financeData.length),
      fundraisingTrend: financeData.length > 4 ? 
        (financeData.slice(-4).reduce((sum, d) => sum + d.totalRaised, 0) > 
         financeData.slice(-8, -4).reduce((sum, d) => sum + d.totalRaised, 0) ? 'increasing' : 'decreasing') : 'stable',
      smallDollarPercentage: Math.round(
        (financeData.reduce((sum, d) => sum + d.smallDollarContributions, 0) / 
         financeData.reduce((sum, d) => sum + d.totalRaised, 0)) * 100
      )
    };
