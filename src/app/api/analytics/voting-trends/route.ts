...
function generateVotingTrends(bioguideId: string, years: number = 5): VotingTrendData[] {
  ...
  for (let year = currentYear - years; year <= currentYear; year++) {
    for (let quarter = 1; quarter <= 4; quarter++) {
      ...
      const baseVotes =
        quarter === 2 || quarter === 3 ? random(40, 80, timeOffset) : random(20, 50, timeOffset);
  ...
  try {
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        averageVotesPerPeriod: Math.round(
          trends.reduce((sum, t) => sum + t.totalVotes, 0) / trends.length
        ),
        averagePartyLoyalty: Math.round(
          trends.reduce((sum, t) => sum + t.partyLoyaltyScore, 0) / trends.length
        ),
        trendDirection:
          trends.length > 1
