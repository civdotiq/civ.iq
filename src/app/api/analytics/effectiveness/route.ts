...
function generateEffectivenessData(bioguideId: string, years: number = 8): EffectivenessData[] {
  ...
  for (let year = currentYear - years; year <= currentYear; year++) {
    ...
    const billsSponsored = Math.round(
      random(3, 25, yearOffset) * experienceMultiplier * activityMultiplier
    );
    ...
    const specializations = policyAreas
  ...
  try {
    ...
    const summary = {
      averageScore: Math.round(
        effectivenessData.reduce((sum, d) => sum + d.overallScore, 0) / effectivenessData.length
      ),
      ...
      overallSuccessRate: Math.round(
        (effectivenessData.reduce((sum, d) => sum + d.billsEnacted, 0) /
          effectivenessData.reduce((sum, d) => sum + d.billsSponsored, 0)) *
          100
      ),
      scoresTrend:
        effectivenessData.length > 3
          ? effectivenessData.slice(-3).reduce((sum, d) => sum + d.overallScore, 0) / 3 >
            effectivenessData.slice(0, 3).reduce((sum, d) => sum + d.overallScore, 0) / 3
            ? 'improving'
            : 'declining'
          : 'stable',
      ...
      topSpecializations:
        effectivenessData[effectivenessData.length - 1]?.specializations.slice(0, 3) || [],
    };
