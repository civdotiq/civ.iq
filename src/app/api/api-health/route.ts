  ...
  try {
    ...
    if (response.ok) {
      ...
      if (contentType?.includes('application/json')) {
        ...
        check.details = {
          hasData: !!data,
          sampleSize: Array.isArray(data) ? data.length : 
                      data.results ? data.results.length : 
                      data.members ? data.members.length : 
                      data.bills ? data.bills.length : 1
        }
      }
    } else {
...
export async function GET(request: NextRequest) {
  try {
    structuredLogger.info('Running API health checks', { 
    ...
    structuredLogger.info('API health check completed', {
      operation: 'api_health_check',
    ...
    structuredLogger.error('API health check failed', error as Error, {
      operation: 'api_health_check'
    }, request)
