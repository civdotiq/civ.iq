...
export async function GET(
  ...
  try {
    ...
    logger.info('Enhanced finance API called', { bioguideId });

    // Try to get FEC ID from bioguide mapping
    const fecId = getFECIdFromBioguide(bioguideId);

    if (!fecId) {
      logger.warn('No FEC ID found for bioguide', { bioguideId });
    ...
    try {
      ...
      logger.error('Error processing comprehensive FEC data', processorError as Error, {
        bioguideId,
        fecId,
      });
