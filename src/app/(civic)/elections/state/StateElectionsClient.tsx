...
interface StateElectionsClientProps {
  /** 2024 governor results rendered on the server for the default view. */
  initialResults: RaceResultFull[];
}

export default function StateElectionsClient({ initialResults }: StateElectionsClientProps) {
  ...
  const [results, setResults] = useState<RaceResultFull[]>(initialResults);
  // Without server data, show loading rather than a "0 races" empty state
  const [loading, setLoading] = useState(initialResults.length === 0);
  const [error, setError] = useState<string | null>(null);
  // The server already rendered the default view; don't refetch it on mount
  const seededDefaultView = useRef(initialResults.length > 0);
  ...
  useEffect(() => {
    const isDefaultView = activeTab === 'governor' && year === '2024' && !stateFilter && !chamber;
    if (seededDefaultView.current && isDefaultView) {
      seededDefaultView.current = false;
      return;
    }
    seededDefaultView.current = false;
    fetchResults();
  }, [fetchResults, activeTab, year, stateFilter, chamber]);
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        {stateFilter &&
          ELECTION_2024_METADATA.missingStates.includes(stateFilter) &&
          (activeTab !== 'governor' || year === '2024') &&
          !loading && (
            <div className="border-2 border-gray-300 dark:border-gray-600 p-8 text-center mb-6">
              <p className="text-gray-600 dark:text-gray-300 font-medium mb-2">
                {US_STATES.find(s => s.code === stateFilter)?.name || stateFilter} is not yet
