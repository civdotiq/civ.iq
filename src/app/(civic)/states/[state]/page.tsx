...
export default async function StateOverviewPageRoute({ params, searchParams }: StatePageProps) {
  ...
  if (!stateCode || !stateName) {
    return (
      <CqPage currentNav="states" crumbs={['CIV.IQ', 'States', state.toUpperCase()]}>
  ...
  return (
    <Suspense fallback={<RedesignLoading stateCode={stateCode} stateName={stateName} />}>
      <RedesignedStateContent stateCode={stateCode} stateName={stateName} />
    </Suspense>
  );
}

async function RedesignedStateContent({
  stateCode,
  stateName,
}: {
  stateCode: string;
  ...
  return (
    <CqPage currentNav="states" crumbs={['CIV.IQ', 'States', stateName]}>
      <StateOverview data={data} />
    </CqPage>
  );
}

function RedesignLoading({ stateCode, stateName }: { stateCode: string; stateName: string }) {
  return (
    <CqPage currentNav="states" crumbs={['CIV.IQ', 'States', stateName]}>
      <div style={{ padding: '32px 36px 56px', maxWidth: 1280, margin: '0 auto' }}>
        <p
