    ...
    <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-all transform hover:-translate-y-1">
      <div className="p-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              const currentCompare = new URLSearchParams(window.location.search).get('compare') || '';
...
function FilterSidebar({ 
  ...
  const committees = Array.from(new Set(representatives.flatMap(r => r.committees.map(c => c.name)))).sort();
...
function RepresentativesPageContent() {
  ...
  const handleFilterChange = (filters: any) => {
    ...
    if (filters.committee !== 'all') {
      filtered = filtered.filter(r => 
        r.committees.some(c => c.name === filters.committee)
      );
    }
  ...
  const generateVotingData = () => {
    const bills = ['HR 1234', 'S 5678', 'HR 9012', 'S 3456', 'HR 7890', 'S 2345', 'HR 6789', 'S 1234'];
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-blue-700 mt-1">
                  {compareIds.length === 2 ? 'Click "View Comparison" to see detailed analysis' : 'Select one more representative to compare'}
                </p>
              </div>
              <div className="flex gap-2">
                {compareIds.length === 2 && (
