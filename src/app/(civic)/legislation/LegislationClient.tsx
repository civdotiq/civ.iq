...
interface CongressBill {
  congress: number;
  // Null on corpus rows when upstream records no action yet
  latestAction: {
    actionDate: string;
    text: string;
  } | null;
  ...
  title: string;
  type: string;
  // Absent on policy-area results, which come from the GovInfo corpus
  updateDate?: string;
  policyArea?: {
    name: string;
  };
...
interface BillsApiResponse {
  ...
  metadata: {
    congress: number;
    // Full policy-area count when filtered; null when the corpus is unavailable
...
function LegislationContent({ policyAreas }: { policyAreas: string[] }) {
  ...
  // Population behind the list when a topic is set (not the fetch cap)
  const [topicTotal, setTopicTotal] = useState<number | null>(null);
  const [topicNote, setTopicNote] = useState<string | null>(null);

  // Congress.gov's bill list carries no policy area, so a topic filter is a
  // server query against the policy-area corpus, not a client-side filter.
  useEffect(() => {
    ...
    async function fetchBills() {
      try {
        setLoading(true);
        setError(null);
        const url = policyAreaFilter
          ? `/api/bills/latest?limit=50&policyArea=${encodeURIComponent(policyAreaFilter)}`
          : '/api/bills/latest?limit=50';
        ...
        setTopicTotal(policyAreaFilter ? data.metadata.totalBills : null);
        setTopicNote(data.metadata.note ?? null);
      } catch {
  ...
  }, [bills, searchQuery, chamberFilter, typeFilter]);
      ...
      <main className="min-h-screen px-4 pt-8 pb-16 bg-white">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white border-2 border-black p-4 sm:p-6 mb-8">
            ...
            {(filteredBills.length !== bills.length || policyAreaFilter) && (
              <div className="mt-4 text-sm text-gray-600">
                Showing {filteredBills.length} of {bills.length} loaded bills
                {topicTotal !== null &&
                  ` · ${topicTotal.toLocaleString()} bills in this topic, most recent action first`}
                {policyAreaFilter && (
          ...
          {loading ? (
            ...
            <div className="bg-white border-2 border-black p-12 text-center">
              ...
              <p className="text-gray-600">
                {topicNote ??
                  (searchQuery || chamberFilter || typeFilter || policyAreaFilter
                    ...
                    <div className="flex-1">
                      ...
                      <div className="bg-gray-50 p-3 mb-3">
                        ...
                        {bill.latestAction ? (
                          ...
                          <p className="text-sm text-gray-600">No action recorded yet</p>
                        )}
                      </div>
            ...
            <div className="mt-8 text-center text-sm text-gray-500">
              <p className="flex items-center justify-center gap-2">
                ...
                {policyAreaFilter ? (
                  ...
                  >
                    GovInfo BILLSTATUS (CRS policy areas)
          ...
          <div className="mt-16 bg-white border-2 border-black p-4 sm:p-8">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 max-w-5xl mx-auto">
              <div className="text-center">
                <div className="w-16 h-16 bg-gray-200 flex items-center justify-center mx-auto mb-2">
...
export default function LegislationClient({ policyAreas }: { policyAreas: string[] }) {
  return (
    <Suspense
      fallback={
    ...
    >
      <LegislationContent policyAreas={policyAreas} />
    </Suspense>
  );
}
