...
export function loadImageWithFallbacks(
  ...
  return new Promise((resolve, reject) => {
    ...
    function tryNextImage() {
      ...
      let imageTimeout: NodeJS.Timeout | undefined;
      ...
      img.onerror = () => {
        cleanup();
        // Image failed to load, trying next source
        currentIndex++;
      ...
      imageTimeout = setTimeout(() => {
        cleanup();
        // Image loading timed out, trying next source
        currentIndex++;
        setTimeout(tryNextImage, 100);
      }, individualTimeoutMs);
      ...
      try {
        ...
        cleanup();
        // Error setting image source, trying next
        currentIndex++;
        setTimeout(tryNextImage, 100);
      }
    }
