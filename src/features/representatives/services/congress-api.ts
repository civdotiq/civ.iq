...
export function formatCongressMember(member: CongressApiMember): Representative {
  ...
  return {
    ...
    terms:
      member.terms?.item?.map((term: unknown) => ({
...
export async function getRepresentativesByLocation(
  ...
  if (stateSenators.length > 0) {
    structuredLogger.info('Adding senators from hardcoded data', {
      component: 'congressApi',
      metadata: { count: stateSenators.length, state },
    });
    ...
    TN: [
      {
        ...
        chamber: 'Senate',
        phone: '(202) 224-4944',
        website: 'https://www.alexander.senate.gov',
        yearsInOffice: 18,
        nextElection: '2026',
      },
      {
