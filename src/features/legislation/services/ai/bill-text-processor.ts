...
export class BillTextProcessor {
  ...
  private static readonly PATTERNS = {
    ...
    lineNumber: /^\s*\d+\s+/gm,
    boilerplate:
      /(Be it enacted by the Senate and House of Representatives[^.]*\.)|(\d+th CONGRESS \d+st Session)/gi,
  };

  /**
   * Process full bill text into structured, analyzable format
   */
  static async processBillText(
    rawText: string,
    billMetadata?: {
      number?: string;
      title?: string;
  ...
  private static extractTextChunks(text: string): BillTextChunk[] {
    ...
    return chunks.filter(
      chunk => chunk.text.length >= this.MIN_CHUNK_SIZE && chunk.text.length <= this.MAX_CHUNK_SIZE
    );
  }
