...
export class ReadingLevelValidator {
  ...
  // Complex words from PlainLanguage.gov word substitution list + legislative jargon
  private static readonly COMPLEX_WORDS_8TH_GRADE = [
    ...
    'verification',
    'validation',
    // PlainLanguage.gov word list additions
    'commence',
    'utilize',
    'promulgate',
    'notwithstanding',
  ...
  // Plain-language alternatives per PlainLanguage.gov word substitution list
  private static readonly WORD_SIMPLIFICATIONS: Record<string, string> = {
    // Original legislative jargon
    legislation: 'law',
    ...
    // PlainLanguage.gov federal word substitutions
    commence: 'begin',
    utilize: 'use',
    promulgate: 'issue',
    ...
    inaugurate: 'begin',
    necessitate: 'require',
    promulgation: 'issuing',
    stipulate: 'require',
    supersede: 'replace',
    statutory: 'legal',
    codified: 'written into law',
    rescind: 'cancel',
    amortization: 'paying off over time',
    appropriated: 'set aside',
    enacted: 'passed into law',
    fiscal: 'financial',
    // Additional PlainLanguage.gov substitutions
    terminate: 'end',
    initiate: 'start',
    compensate: 'pay',
    subsequent: 'next',
  ...
  static analyzeReadingLevel(
    ...
    try {
      ...
      const suggestions = provideSuggestions
        ? this.generateSuggestions(text, stats, complexWords)
        : [];
  ...
  private static generateSuggestions(
    ...
    if (complexWords.length > 10) {
      suggestions.push(
        'Too many complex words. Replace difficult terms with simpler alternatives.'
      );
    ...
    const wordsWithReplacements = complexWords.filter(word => this.WORD_SIMPLIFICATIONS[word]);

    if (wordsWithReplacements.length > 0) {
      const examples = wordsWithReplacements
        .slice(0, 3)
        .map(word => `"${word}" → "${this.WORD_SIMPLIFICATIONS[word]}"`);
  ...
  static validateBatch(
    ...
    for (const { id, content } of texts) {
      try {
        ...
        results.set(
          id,
          this.generateFallbackAnalysis(content, options.targetGrade || this.DEFAULT_TARGET_GRADE)
        );
      }
    }
