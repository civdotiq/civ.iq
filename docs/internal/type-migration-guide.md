...
export class LegacyTypeAdapter {
   ...
   */
  static toLegacyRepresentativeResponse(unified: UnifiedRepresentativeResponse): any {
    return {
      bioguideId: unified.bioguideId,
      name: unified.name,
      party: unified.party === 'Democratic' ? 'D' : unified.party === 'Republican' ? 'R' : 'I',
      state: unified.state,
   ...
   */
  static toLegacyApiResponse<T>(unified: ApiResponse<T>): any {
    return {
      data: unified.data,
