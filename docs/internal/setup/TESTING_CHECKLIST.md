...
- [ ] Initial page load time: **\_** seconds
- [ ] Batch API response time: **\_** seconds
- [ ] Total API calls made: **\_** (should be 1)
- [ ] Tab switch time: **\_** ms (should be instant)
...
window.performance
  ...
  .filter(
    r =>
      ...
      (r.name.includes('/bills') ||
        r.name.includes('/finance') ||
        r.name.includes('/committees') ||
        r.name.includes('/voting-record'))
  );
// Should return empty array []

// Check batch API calls
window.performance.getEntriesByType('resource').filter(r => r.name.includes('/batch'));
