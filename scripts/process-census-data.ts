...
async function processCensusData() {
  ...
  const inputFile = path.join(process.cwd(), 'data', 'tab20_zcta520_cd119_natl.txt');
  const outputFile = path.join(process.cwd(), 'src', 'lib', 'data', 'complete-zip-mapping.ts');

  if (!fs.existsSync(inputFile)) {
    console.error(`Input file not found: ${inputFile}`);
    console.log(
      'Please download from: https://www2.census.gov/geo/docs/maps-data/data/rel2020/zcta520/'
    );
    process.exit(1);
  }
  ...
  for await (const row of parser) {
    ...
    try {
      ...
      const district = row.CD119.padStart(2, '0');
