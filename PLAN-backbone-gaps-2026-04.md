...
| 4 — Scope honesty        | 🟢 done                                   | 2026-04-16 | see commits                                        | **Decisions:** state finance = Option C short-term + A tracked (document as out-of-scope, track OpenSecrets/FTM merger); local government = Option C (immediate B + ongoing A) — relabel as "federal + state + 10 pilot cities," Legistar expansion lives in a separate roadmap. **`docs/COVERAGE.md`** is now the canonical coverage matrix (federal/state/local × domains, dated 2026-04-16) — every other public claim defers to that page. README, `public/llms.txt`, About page (`src/app/(public)/about/page.tsx`), and `/local` page rewritten to match: Campaign Finance is "Federal only," State Legislatures explicitly excludes finance/lobbying/stock trades, Local Government enumerates the 10 pilot cities by name. **State finance route** (`src/app/api/state-legislature/[state]/legislator/[id]/finance/route.ts`) rewritten as a `BackboneResponse` — when `FOLLOWTHEMONEY_API_KEY` is absent (current reality) it returns 503 with `dataQuality: 'unavailable'` and `sourceStatus: [{ source: 'followthemoney', status: 'not-configured', ... }]` plus a note citing the merger; on the configured path uses `fetchWithSourceStatus()` + `computeDataQuality()` consistent with Phase 2 routes. **State legislator UI** (`SimpleStateLegislatorProfile.tsx`) replaces the stale "coming soon" placeholder with an honest unavailable card (amber `AlertCircle`, FTM/OpenSecrets explanation, link to COVERAGE.md). Tab description updated to "Campaign finance data (currently unavailable)." **Local government:** extracted `CITY_CONFIGS` to `src/lib/local-government/pilot-cities.ts` so the council route and the local-gov catch-all share one source of truth; `/api/local-government/[location]` rewritten to return a `BackboneResponse` — pilot-city matches return 200 + `dataQuality: 'partial'` pointing the caller to `/api/city/[id]/council`, everything else returns 503 + `dataQuality: 'unavailable'` with a `pilotCities[]` payload listing all 10 supported cities and their endpoints (no more silent empty `officials: []`, no more fake city/county data). `validate:all` green: 6 passed / 2 warn (carried-over Phase 1 install-time CVEs) / 0 fail; type-check clean; verification greps confirm no overclaim survives in README/llms.txt/about.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
