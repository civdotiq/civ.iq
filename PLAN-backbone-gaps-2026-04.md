...
| 2 — Integrity core       | 🟢 done        | 2026-04-16 | see commits                                       | BackboneResponse contract (`src/types/backbone-response.ts`) with `DataQuality` enum, `SourceStatus`, `fetchWithSourceStatus()` helper. Applied to committees, spending, lobbying routes — all now return `dataQuality` + `sourceStatus[]` instead of silent `[]`. Lobbying matcher expanded from 13 to 30 committee keyword entries covering all House + Senate standing committees, plus embedding-based fallback via `embedText()` + cosine similarity (threshold 0.45). `matchingMethod` and `matchConfidence` added to `CommitteeLobbyingData`. UI updated: `SpendingSummaryCards` handles `unavailable` state, `LobbyingTab` surfaces metadata notes. 11 new integration tests verify each route returns `dataQuality: 'unavailable'` on upstream failure. Also fixed dompurify 3.3.2→3.4.0 (moderate CVE). `validate:all` green: 186 suites, 2342 tests, 0 failures.      |
